...
export default function CategoryListView() {
  ...
  const { categorys, refreshCategorys } = useGetCategorys();
  ...
  const handleEditRow = useCallback(
    ...
    (id) => {
      router.push(paths.dashboard.category.view(id));
    },
    [router]
  );
              ...
              <Tab
                ...
                icon={
                  ...
                  >
                    ...
                    {tab.value === '1' && tableData.filter((category) => category.isActive).length}

                    {tab.value === '0' && tableData.filter((category) => !category.isActive).length}
                  </Label>
                }
              />
            ))}
  ...
  if (role.length) {
    inputData = inputData.filter(
      (category) =>
        ...
        category.permissions.some((categoryRole) => {
          const mappedRole = roleMapping[categoryRole];
