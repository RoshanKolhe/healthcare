...
export default function HospitalServiceViewForm({ currentHospitalService }) {
  const theme = useTheme();
  const isDark = theme.palette.mode === 'dark';
  
  const NewDepartmentSchema = Yup.object().shape({
    hospitalService: Yup.string().required('HospitalService  is required'),
        ...
        <Card sx={{ pb: 2 }}>
          <Stack spacing={3} sx={{ p: 3 }}>
            <Grid container spacing={2} xs={12} md={12}>
              ...
              <Grid xs={12} md={6}>
                <RHFTextField name="hospitalService" label="HospitalService" disabled/>
              </Grid>
              <Grid xs={12} md={6}>
...
HospitalServiceViewForm.propTypes = {
