...
import PlanQuickEditForm from '../plan-quick-edit-form';
...
const TABLE_HEAD = [
  ...
  { id: 'billingCycle', label: 'Billing Cycle', width: 120 },
  { id: 'priceINR', label: 'Price (INR)', width: 120 },
  { id: 'discountedPriceINR', label: 'Discounted Price (INR)', width: 150 },
  { id: 'priceUSD', label: 'Price (USD)', width: 120 },
  { id: 'discountedPriceUSD', label: 'Discounted Price (USD)', width: 150 },
  { id: 'bookingLimit', label: 'Booking Limit', width: 120 },
...
export default function PlanListView() {
  ...
  const handleViewRow = useCallback(
    (id) => {
      router.push(paths.dashboard.plan.view(id));
    },
    [router]
  );
  ...
  // }, [plans]);
  useEffect(() => {
    if (plans && clinics.length) {
      const enrichedPlanes = plans.map((plan) => {
        const matchedClinic = clinics.find((h) => h.id === plan.clinicId);
        return {
          ...plan,
          clinicName: matchedClinic?.clinicName || 'N/A',
        };
      });
      setTableData(enrichedPlanes);
    }
  }, [plans, clinics]);
      ...
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Plan', href: paths.dashboard.plan.list },
            { name: 'List' },
          ]}
          action={
            isSuperOrAdmin ? (
              ...
              <Tab
                ...
                icon={
                  ...
                  >
                    ...
                    {tab.value === '1' && tableData.filter((plan) => plan.isActive).length}

                    {tab.value === '0' && tableData.filter((plan) => !plan.isActive).length}
                  </Label>
                }
              />
            ))}
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter((plan) =>
      Object.values(plan).some((value) => String(value).toLowerCase().includes(name.toLowerCase()))
    );
  }
  ...
  //   inputData = inputData.filter((plan) => {
  //     if (status === '1') return plan.status === 1;
  //     if (status === '2') return plan.status === 2;
  ...
  if (role.length) {
    inputData = inputData.filter(
      (plan) =>
        plan.permissions &&
        plan.permissions.some((planRole) => {
          const mappedRole = roleMapping[planRole];
