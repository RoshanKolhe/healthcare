...
export default function AgentListView() {
  ...
  const { agents, agentsLoading, agentsEmpty, refreshAgents } = useGetAgents();
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.agent.edit(id));
    },
    [router]
  );
  ...
  const handleViewRow = useCallback(
    (id) => {
      router.push(paths.dashboard.agent.view(id));
    },
    [router]
  );
  ...
  // }, [agents]);
  useEffect(() => {
    if (agents && clinics.length) {
      const enrichedAgentes = agents.map((agent) => {
        const matchedClinic = clinics.find((h) => h.id === agent.clinicId);
        return {
          ...agent,
          clinicName: matchedClinic?.clinicName || 'N/A',
        };
      });
      setTableData(enrichedAgentes);
    }
  }, [agents, clinics]);
      ...
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Agent', href: paths.dashboard.agent.list },
            { name: 'List' },
          ]}
          action={
            isSuperOrAdmin ? (
              ...
              <Tab
                ...
                icon={
                  ...
                  >
                    ...
                    {tab.value === '1' && tableData.filter((agent) => agent.isActive).length}

                    {tab.value === '0' && tableData.filter((agent) => !agent.isActive).length}
                  </Label>
                }
              />
            ))}
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter((agent) =>
      Object.values(agent).some((value) => String(value).toLowerCase().includes(name.toLowerCase()))
    );
  }
  ...
  //   inputData = inputData.filter((agent) => {
  //     if (status === '1') return agent.status === 1;
  //     if (status === '2') return agent.status === 2;
  ...
  if (role.length) {
    inputData = inputData.filter(
      (agent) =>
        agent.permissions &&
        agent.permissions.some((agentRole) => {
          const mappedRole = roleMapping[agentRole];
