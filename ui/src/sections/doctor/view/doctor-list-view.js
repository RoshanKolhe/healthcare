...
import DoctorQuickEditForm from '../doctor-quick-edit-form';
...
export default function DoctorListView() {
  ...
  const { doctors, refreshDoctors } = useGetDoctors();
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.doctor.edit(id));
    },
    [router]
  );
  ...
  const handleViewRow = useCallback(
    (id) => {
      router.push(paths.dashboard.doctor.view(id));
    },
    [router]
  );
  ...
  useEffect(() => {
    if (doctors) {
      const updatedDoctors = doctors.filter((obj) => !obj.permissions.includes('super_admin'));
      ...
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Doctor', href: paths.dashboard.doctor.list },
            { name: 'List' },
          ]}
          action={
            <Button
              component={RouterLink}
              href={paths.dashboard.doctor.new}
              variant="contained"
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (role.length) {
    inputData = inputData.filter((doctor) => role.includes(doctor.role));
  }
