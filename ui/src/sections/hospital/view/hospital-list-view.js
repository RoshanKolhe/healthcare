...
//
import { useGetHospitals } from 'src/api/hospital';
...
import HospitalTableFiltersResult from '../hospital-table-filters-result';
...
export default function HospitalListView() {
  ...
  const handleEditRow = useCallback(
    ...
    (id) => {
      router.push(paths.dashboard.hospital.view(id));
    },
    [router]
  );
              ...
              <Tab
                ...
                icon={
                  ...
                  >
                    ...
                    {tab.value === '1' && tableData.filter((hospital) => hospital.isActive).length}

                    {tab.value === '0' && tableData.filter((hospital) => !hospital.isActive).length}
                  </Label>
                }
              />
            ))}
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter((hospital) =>
  ...
  //   inputData = inputData.filter((hospital) => {
  //     if (status === '1') return hospital.status === 1;
  //     if (status === '2') return hospital.status === 2;
  //     return hospital.status === 0;
  //   });
  // }
  if (status !== 'all') {
  ...
  if (role.length) {
    inputData = inputData.filter(
      (hospital) =>
        ...
        hospital.permissions.some((hospitalRole) => {
          const mappedRole = roleMapping[hospitalRole];
