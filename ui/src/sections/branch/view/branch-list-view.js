...
const { filteredhospitals: hospitals = [] } = useGetHospitalsWithFilter(hospitalFilter);
  ...
  const handleEditRow = useCallback(
    ...
    (id) => {
      router.push(paths.dashboard.branch.view(id));
    },
    [router]
  );
  ...
  return (
    <>
              ...
              <Tab
                ...
                icon={
                  ...
                  >
                    ...
                    {tab.value === '1' && tableData.filter((branch) => branch.isActive).length}

                    {tab.value === '0' && tableData.filter((branch) => !branch.isActive).length}
                  </Label>
                }
              />
            ))}
      ...
      {quickEdit.value && quickEditRow && (
        <BranchQuickEditForm
          currentBranch={quickEditRow}
          open={quickEdit.value}
          onClose={() => {
            setQuickEditRow(null);
            quickEdit.onFalse();
          }}
          refreshBranchs={refreshBranchs}
        />
      )}
    </>
  );
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  //   inputData = inputData.filter((branch) => {
  //     if (status === '1') return branch.status === 1;
  //     if (status === '2') return branch.status === 2;
  ...
  if (role.length) {
    inputData = inputData.filter(
      (branch) =>
        branch.permissions &&
        branch.permissions.some((branchRole) => {
          const mappedRole = roleMapping[branchRole];
